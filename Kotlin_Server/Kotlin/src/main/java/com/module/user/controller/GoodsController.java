...
public class GoodsController extends BaseController {
    ...
    @RequestMapping(value = {"/getGoodsList"}, method =
    ...
    public BaseResp<List<GoodsInfo>> getGoodsList(@RequestBody GetGoodsListReq req) {
        ...
        List<GoodsInfo> list =
                this.goodsService.getGoodsList(req.getCategoryId(),
                        req.getPageNo());
    ...
    @RequestMapping(value = {"/getGoodsDetail"}, method =
            {org.springframework.web.bind.annotation.RequestMethod.POST})
    @ResponseBody
    public BaseResp<GoodsInfo> getGoodsDetail(@RequestBody GetGoodsDetailReq req) {
        ...
        GoodsInfo goodsInfo =
                this.goodsService.getGoodsDetail(req.getGoodsId());
        if (goodsInfo == null) {
            resp.setStatus(0);
        ...
        List<GoodsSku> skuList =
                this.goodsService.getGoodsSkuList(goodsInfo.getId());
    ...
    @RequestMapping(value = {"/getGoodsListByKeyword"}, method =
            {org.springframework.web.bind.annotation.RequestMethod.POST})
    @ResponseBody
    public BaseResp<List<GoodsInfo>> getGoodsListByKeyword(@RequestBody GetGoodsListByKeywordReq req) {
        ...
        List<GoodsInfo> list =
                this.goodsService.getGoodsListByKeyword(req.getKeyword(),
                        req.getPageNo());
        if ((list == null) || (list.size() == 0)) {
            resp.setStatus(0);
            resp.setMessage("列表为空");
